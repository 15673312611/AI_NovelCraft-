...
import novelVolumeService, { NovelVolume, VolumeGuidanceRequest } from '../frontend/src/services/novelVolumeService';
import { withAIConfig, checkAIConfig, AI_CONFIG_ERROR_MESSAGE } from '../frontend/src/utils/aiRequest';
import './.css';
                          ...
                          for (const line of lines) {
                            ...
                            if (line.startsWith('data:')) {
                              ...
                              if (shouldAddContent && contentToAdd) {
                                ...
                                // 3. 智能换行：保护引号内对话完整性（支持流式输出未闭合引号）
                                // 关键：
                                // - 引号内的标点不换行（"你好！" 中的！不换行）
                                // - 标点后紧跟引号不换行（他说。"你好 中的。后不换行）
                                // - 只在真正的句末（引号外且后面不是引号）才换行
                                // 流式场景：一个字一个字返回，需要实时判断状态
                                ...
                                for (let i = 0; i < displayContent.length; i++) {
                                  ...
                                  const nextChar = displayContent[i + 1];
                                  ...
                                  if (char === '"') {
                                    ...
                                    // 如果是闭合引号（从引号内出来），且后面有内容且不是换行，则在引号后换行
                                    if (!inQuote && nextChar && nextChar !== '\n') {
                                      result += '\n';
                                    }
                                  }
                                  // 如果是句末标点（。！？）
