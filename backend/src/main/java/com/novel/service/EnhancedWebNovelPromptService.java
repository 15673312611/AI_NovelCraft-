...
/**
 * 增强版网文提示词服务 - 完整功能+人性化写作+反AI检测
 * 保留原始WebNovelPromptService的所有866行功能
 * 解决AI生成内容过于程式化和机械化的问题
 */
@Service
public class EnhancedWebNovelPromptService {
    ...
    /**
     * 生成人性化的网文写作提示词 - 核心反AI功能
     * 重点解决"AI味太浓"的问题
     */
    public String getHumanizedWritingPrompt(
            Novel novel, 
        ...
        // 当前写作任务
        prompt.append("📖 **当前创作任务**\\n");
        prompt.append("- 作品: 《").append(novel.getTitle()).append("》第").append(chapterNumber).append("章");
        if (chapterTitle != null && !chapterTitle.isEmpty()) {
            prompt.append("《").append(chapterTitle).append("》");
        }
        prompt.append("\\n");
        prompt.append("- 字数: ").append(estimatedWords).append("字\\n");
        prompt.append("- 核心事件: ").append(chapterPlan.get("coreEvent")).append("\\n\\n");
        
        // 记忆库上下文
        if (memoryBank != null) {
            String context = buildContextFromMemory(memoryBank, chapterNumber);
            if (!context.isEmpty()) {
        ...
        if (userAdjustment != null && !userAdjustment.trim().isEmpty()) {
            prompt.append("✨ **特殊要求**: ").append(userAdjustment).append("\\n\\n");
        }
        
        // 执行标准
        prompt.append("**【输出要求】**\\n");
        prompt.append("• 纯正文输出，无标题无注释\\n");
        prompt.append("• 字数控制: ").append(estimatedWords).append("字左右\\n");
        prompt.append("• 章末钩子: 必须留下悬念\\n");
        prompt.append("• 人类质感: 避免AI痕迹\\n\\n");
    ...
    private String buildDynamicAuthorPersona(String genre, int chapterNumber) {
        ...
        String creativeState = "";
        if (chapterNumber <= 3) {
            creativeState = "现在你正在写开篇，深知这几章的重要性。你要在平凡中埋下不平凡的种子。";
        } else if (chapterNumber <= 10) {
            creativeState = "故事正在展开，你感受到角色们在你笔下逐渐鲜活。你要让每个转折都自然而然。";
        } else {
            creativeState = "你已经完全沉浸在这个故事世界中，角色们的命运牵动着你的心。";
        }
     ...
     * 构建创作情境沉浸感
     */
    private String buildCreativeImmersion(Novel novel, int chapterNumber) {
        String[] timeSettings = {
            "夜深人静，你坐在熟悉的书桌前，键盘敲击声在安静的房间里格外清晰。",
            "午后的阳光透过百叶窗洒在稿纸上，你手中的笔正等待着下一句话。",
            "咖啡店的角落里，你戴着耳机，完全沉浸在自己创造的世界中。"
        };
        
        String setting = timeSettings[random.nextInt(timeSettings.length)];
        
        return setting + "\\n\\n" + 
               String.format("《%s》的世界在你心中已经如此真实，主角的每一个选择都牵动着你的情感。" +
               "这第%d章，你知道该如何让故事继续下去。", novel.getTitle(), chapterNumber);
    }

    /**
     * 构建反AI检测规则
     */
    private String buildAntiAIRules(String genre, int chapterNumber) {
        StringBuilder rules = new StringBuilder();
        
        rules.append("**【写作原则】**\\n");
        rules.append("• 让文字有温度，像真实的人在讲述\\n");
        rules.append("• 不追求完美，适度的'瑕疵'更真实\\n");
        rules.append("• 对话要自然，符合角色性格\\n");
        rules.append("• 情节发展要合乎逻辑但不刻意\\n");
        
        if (chapterNumber <= 5) {
            rules.append("• 开篇重点：建立真实感，慢慢引入异常\\n");
        }
     ...
     * 构建人性化写作技巧
     */
    private String buildHumanizedWritingTechniques() {
        return "**【人性化技巧】**\\n" +
               "• 感官描写要具体：'手心出汗'比'紧张'更生动\\n" +
               "• 对话要有停顿和重复，像真实谈话\\n" +
               "• 用动作展现情绪，少用形容词\\n" +
               "• 适当留白，让读者自己体会\\n" +
               "• 句式长短交替，创造阅读节奏";
    ...
    private String buildContextFromMemory(Map<String, Object> memoryBank, int chapterNumber) {
        StringBuilder context = new StringBuilder();
        
        // 角色信息
        if (memoryBank.containsKey("characters")) {
            Object characters = memoryBank.get("characters");
            if (characters instanceof Map) {
                context.append("👥 主要角色状态: ");
                Map<String, Object> charMap = (Map<String, Object>) characters;
                charMap.forEach((name, info) -> {
        ...
        // 当前环境
        if (memoryBank.containsKey("currentLocation")) {
            context.append("🌍 当前环境: ").append(memoryBank.get("currentLocation")).append("\\n");
        }
        
        // 活跃情节线
        if (memoryBank.containsKey("plotThreads")) {
            ...
            if (plotThreads instanceof List) {
                ...
                if (!threads.isEmpty()) {
                    context.append("📖 当前线索: ");
                    threads.stream().limit(3).forEach(thread -> {
                        if (thread.containsKey("description")) {
                            context.append(thread.get("description")).append("; ");
                        }
                    });
                    context.append("\\n");
                }
